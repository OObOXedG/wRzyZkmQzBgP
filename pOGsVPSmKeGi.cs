...
using System; // BY SONOTCLOSE (@SONOTCLOSE)
using System.Collections; // BY SONOTCLOSE (@SONOTCLOSE)
using System.Collections.Generic; // BY SONOTCLOSE (@SONOTCLOSE)
using System.Text.RegularExpressions; // BY SONOTCLOSE (@SONOTCLOSE)
using ExitGames.Client.Photon; // BY SONOTCLOSE (@SONOTCLOSE)
using GorillaNetworking; // BY SONOTCLOSE (@SONOTCLOSE)
using Photon.Pun; // BY SONOTCLOSE (@SONOTCLOSE)
using Photon.Realtime; // BY SONOTCLOSE (@SONOTCLOSE)
using PlayFab; // BY SONOTCLOSE (@SONOTCLOSE)
using PlayFab.ClientModels; // BY SONOTCLOSE (@SONOTCLOSE)
using Steamworks; // BY SONOTCLOSE (@SONOTCLOSE)
using UnityEngine; // BY SONOTCLOSE (@SONOTCLOSE)
using UnityEngine.UI; // BY SONOTCLOSE (@SONOTCLOSE)
...
public class CosmeticsController : MonoBehaviour
{
    ...
    public void Awake() // BY SONOTCLOSE (.gg/zenunity)
    {
        ...
        if (base.gameObject.activeSelf)
        {
            ...
            itemLists[3] = unlockedHands;
            SwitchToStage(ATMStages.Unavailable);
            StartCoroutine(CheckCanGetDaily());
        }
    }

    public void Update() // BY SONOTCLOSE (.gg/zenunity)
    {
    }

    public void PressFittingRoomButton(FittingRoomButton pressedFittingRoomButton) // BY SONOTCLOSE (.gg/zenunity)
    {
        switch (pressedFittingRoomButton.currentCosmeticItem.itemSlot)
        {
                        ...
                        foreach (string itemID in bundledItems)
                        {
                            ...
                            switch (tempItem.itemSlot)
                            {
                                ...
                                case "hat":
                                    ...
                                    break;
                                case "hand":
                                    tryOnSet.hand = ((tempItem.itemName == tryOnSet.hand.itemName) ? nullItem : tryOnSet.hand);
                                    break;
                            }
                        }
                        break;
            ...
            case "hat":
                ...
                break;
            case "hand":
                tryOnSet.hand = ((pressedFittingRoomButton.currentCosmeticItem.itemName == tryOnSet.hand.itemName) ? nullItem : GetItemFromDict(pressedFittingRoomButton.currentCosmeticItem.itemName));
                break;
        }
        UpdateShoppingCart();
    }
    ...
    public void PressCosmeticStandButton(CosmeticStand pressedStand)
    {
        ...
        if (searchIndex != -1)
        {
            ...
            else if (pressedStand.thisCosmeticItem.itemName == tryOnSet.hand.itemName)
            {
                tryOnSet.hand = nullItem;
            }
        }
    ...
    public void PressWardrobeItemButton(CosmeticItem cosmeticItem) // BY SONOTCLOSE (.gg/zenunity)
    {
        switch (cosmeticItem.itemSlot)
        {
                    ...
                    foreach (string itemID in bundledItems)
                    {
                        ...
                        switch (tempItem.itemSlot)
                        {
                            ...
                            case "hat":
                                ...
                                break;
                            case "hand":
                                currentWornSet.hand = ((tempItem.itemName == currentWornSet.hand.itemName) ? nullItem : tempItem);
                                PlayerPrefs.SetString("handCosmetic", currentWornSet.hand.itemName);
                                tryOnSet.hand = nullItem;
                                break;
                        }
                    }
            ...
            case "hat":
                ...
                break;
            case "hand":
                currentWornSet.hand = ((cosmeticItem.itemName == currentWornSet.hand.itemName) ? nullItem : GetItemFromDict(cosmeticItem.itemName));
                PlayerPrefs.SetString("handCosmetic", currentWornSet.hand.itemName);
                tryOnSet.hand = nullItem;
                break;
        }
    ...
    public void PressWardrobeFunctionButton(string function) // BY SONOTCLOSE (.gg/zenunity)
    {
        switch (function)
        {
            case "badge":
        ...
        else
        {
            ...
            switch (pressedCheckoutCartButton.currentCosmeticItem.itemSlot)
            {
                        ...
                        foreach (string itemID2 in bundledItems)
                        {
                            ...
                            switch (tempItem.itemSlot)
                            {
                                ...
                                case "hat":
                                    ...
                                    break;
                                case "hand":
                                    tryOnSet.hand = ((tempItem.itemName == tryOnSet.hand.itemName) ? nullItem : GetItemFromDict(tempItem.itemName));
                                    break;
                            }
                        }
                        break;
                ...
                case "hat":
                    ...
                    break;
                case "hand":
                    tryOnSet.hand = ((pressedCheckoutCartButton.currentCosmeticItem.itemName == tryOnSet.hand.itemName) ? nullItem : GetItemFromDict(pressedCheckoutCartButton.currentCosmeticItem.itemName));
                    break;
            }
        }
    ...
    public void ProcessPurchaseItemState(string buttonSide) // BY SONOTCLOSE (.gg/zenunity)
    {
        switch (currentPurchaseItemStage) // BY SONOTCLOSE (.gg/zenunity)
        {
            case PurchaseItemStages.Start: 
                itemToBuy = nullItem;
                FormattedPurchaseText("SELECT AN ITEM FROM YOUR CART TO PURCHASE!"); // BY SONOTCLOSE (.gg/zenunity)
                UpdateShoppingCart();
                break;
            case PurchaseItemStages.CheckoutButtonPressed:
                ...
                if (searchIndex > -1)
                {
                    FormattedPurchaseText("YOU ALREADY OWN THIS ITEM!"); // BY SONOTCLOSE (.gg/zenunity)
                ...
                {
                    FormattedPurchaseText("DO YOU WANT TO BUY THIS ITEM?"); // BY SONOTCLOSE (.gg/zenunity)
                ...
                {
                    FormattedPurchaseText("INSUFFICIENT SHINY ROCKS FOR THIS ITEM!"); // BY SONOTCLOSE (.gg/zenunity)
                    leftPurchaseButton.myText.text = "-";
                    rightPurchaseButton.myText.text = "-";
                    leftPurchaseButton.buttonRenderer.material = leftPurchaseButton.pressedMaterial;
                    rightPurchaseButton.buttonRenderer.material = rightPurchaseButton.pressedMaterial;
            ...
            case PurchaseItemStages.FinalPurchaseAcknowledgement:
                if (buttonSide == "left")
                {
                    FormattedPurchaseText("PURCHASING ITEM..."); // BY SONOTCLOSE (.gg/zenunity)
                    leftPurchaseButton.myText.text = "-";
                    rightPurchaseButton.myText.text = "-";
                    leftPurchaseButton.buttonRenderer.material = leftPurchaseButton.pressedMaterial;
                    rightPurchaseButton.buttonRenderer.material = rightPurchaseButton.pressedMaterial;
                ...
                {
                    currentPurchaseItemStage = PurchaseItemStages.CheckoutButtonPressed; // BY SONOTCLOSE (.gg/zenunity)
                    ProcessPurchaseItemState(null);
                }
                break;
        ...
        if (num > -1)
        {
            ...
            switch (allCosmetics[num].itemSlot)
            {
                ...
                case "face":
                    ...
                    break;
                case "hand":
                    unlockedHands.Add(allCosmetics[num]);
                    break;
            }
        }
    }
    ...
    public void UpdateShoppingCart()
    {
        ...
        UpdateWardrobeModelsAndButtons();
        GorillaTagger.Instance.offlineVRRig.LocalUpdateCosmeticsWithTryon(currentWornSet.badge.displayName, currentWornSet.face.displayName, currentWornSet.hand.displayName, currentWornSet.hat.displayName, tryOnSet.badge.displayName, tryOnSet.face.displayName, tryOnSet.hand.displayName, tryOnSet.hat.displayName);
        if (GorillaTagger.Instance.myVRRig != null)
        {
            GorillaTagger.Instance.myVRRig.photonView.RPC("UpdateCosmeticsWithTryon", RpcTarget.All, currentWornSet.badge.displayName, currentWornSet.face.displayName, currentWornSet.hand.displayName, currentWornSet.hat.displayName, tryOnSet.badge.displayName, tryOnSet.face.displayName, tryOnSet.hand.displayName, tryOnSet.hat.displayName);
        }
    }
    ...
    public bool AnyMatch(CosmeticSet set, CosmeticItem item)
    {
        ...
        {
            if (!(set.hat.itemName == item.itemName) &&
                !(set.face.itemName == item.itemName) &&
                !(set.badge.itemName == item.itemName))
            {
                return set.hand.itemName == item.itemName;
            }
            return true;
        }
        ...
        PlayFabClientAPI.GetUserInventory(new GetUserInventoryRequest(), delegate (GetUserInventoryResult result)
        {
            ...
            }, delegate (GetCatalogItemsResult result2)
            {
                ...
                foreach (CosmeticItem unlockedCosmetic in unlockedCosmetics)
                {
                    ...
                    else if (unlockedCosmetic.itemSlot == "hand" && !unlockedHands.Contains(unlockedCosmetic))
                    {
                        unlockedHands.Add(unlockedCosmetic);
                    }
                ...
                currentWornSet.hand = GetItemFromDict(PlayerPrefs.GetString("handCosmetic", "NOTHING"));
    ...
    public string[] SetToStringArray(CosmeticSet set)
    {
        ...
        returnStringArray[3] = set.hand.displayName;
